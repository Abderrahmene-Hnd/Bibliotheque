                ...
                <div class="relative flex h-16 items-center justify-between">
                    <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
                        <div class="flex  items-center">
                            ...
                            @if (
                                $currentLink == '/dashboard/user' ||
                                    $currentLink == '/dashboard/book' ||
                                    $currentLink == '/dashboard/category' ||
                                    $currentLink == '/dashboard/author' ||
                                    $currentLink == '/dashboard' ||
                                    $currentLink == '/settings')
                            @else
                                <a href="/"><img class="pt-2.5 pr-2" src="{{ asset('storage/images/Lybrrarywhite.png') }}"
                                        alt="Lary avatar" width="50"></a>
                                <a href="/" class="text-2xl font-bold {{ $txt_pry_color ?: 'text-white' }} ">My Library
                                </a>
                            @endif
                        ...
                        <div class="hidden sm:ml-6 sm:block">
                            <div class="flex space-x-4 ml-52">
                                @switch($currentLink)
                                    @case('/dashboard/user')
                                        <span class="{{ $txt_pry_color ?: 'text-white' }} text-center text-4xl  ml-96">Users
                                            Manager</span>
                                    @break
                                    ...
                                    @case('')
                                        @auth
                                            <a href="/"
                                                class="{{ $txt_sec_color ?: 'text-white' }} {{ $txt_sec_hov_color ?: 'hover:text-white' }} px-3 py-2  font-medium"
                                                aria-current="page">Vitrine</a>
                                        @endauth
                                        @role('manager|owner')
                                            <a href="/dashboard"
                                                class="{{ $txt_sec_color ?: 'text-gray-300' }} {{ $txt_sec_hov_color ?: 'hover:text-white' }} px-3 py-2 font-medium">Dashbord</a>
                                        @endrole

                                        @role('owner')
                                            <a href="/dashboard/user"
                                                class="{{ $txt_sec_color ?: 'text-gray-300' }} {{ $txt_sec_hov_color ?: 'hover:text-white' }} px-3 py-2 font-medium">Users
                                                Manager</a>
                                        @endrole
                                    @break
                    ...
                    <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                        ...
                        @auth
                            ...
                            <div @click.away="open = false" class="relative" x-data="{ open: false }">
                                ...
                                <div x-show="open" style="display: none" x-transition:enter="transition ease-out duration-100"
                                    ...
                                    x-transition:leave-end="transform opacity-0 scale-95"
                                    class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
                                    <div class=" rounded-md shadow dark-mode:bg-gray-800 bg-gray-100">
                                        @forelse ($notifications as $notification)
                                            <div class="my-2 p-3 hover:bg-gray-300 cursor-pointer flex">
                                                <div>
                                                ...
                                                <div class="p-3">
                                                    <button wire:click="delete"
                                                        class="text-white bg-red-500 items-center border rounded-md p-2">Delete</button>
                                                </div>
                                            </div>
                                        @empty
                                            <div class="px-3 py-2">
                        ...
                        @guest
                            <a href="/login"
                                class="{{ $txt_sec_color ?: 'bg-gray-300' }} {{ $txt_sec_hov_color ?: 'hover:bg-white' }} px-3 py-2  font-medium"
